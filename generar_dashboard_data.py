...
def build_dashboard_data(raw, excel_path=None):
    ...
    # ── Rangos ÚNICOS por programa — leer de hoja S directamente ─────────────
    ...
    if excel_path:
        try:
            ...
            _wb_s = _opx.load_workbook(str(excel_path), data_only=True, read_only=True)
        except Exception:
            _wb_s = None
    if _wb_s and 'Unicos y Rango de Edad' in _wb_s.sheetnames:
        ...
        for row_s in rows_s:
            ...
            hoja_s_rangos[nombre_s.upper()] = rangos_s
            hoja_s_sin[nombre_s.upper()]    = int(sin_d_s) if isinstance(sin_d_s,(int,float)) and sin_d_s else 0
    ...
    for ind in indicadores:
        ...
        nombre_prog_alias = IND_PROG_ALIAS.get(nombre_prog_key, nombre_prog_key)
        _rk2 = ['0-5','6-11','12-17','18-29','30-49','50-64','65+']
        # Rangos desde hoja S (beneficiarios únicos, dato correcto)
        _rp  = hoja_s_rangos.get(nombre_prog_alias, hoja_s_rangos.get(nombre_prog_key, {}))
        _sin_datos_rango = hoja_s_sin.get(nombre_prog_alias, hoja_s_sin.get(nombre_prog_key, 0))
