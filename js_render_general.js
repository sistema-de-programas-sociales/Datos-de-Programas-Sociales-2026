function renderGeneral() {
  ...
  document.getElementById('kpi-general-all').innerHTML =
    kpiSS(
      ...
      fmt(g.total_benef),
      `${pct(g.total_benef, D._meta.pob_vulnerable)} de pob. vulnerable`,
      'cb',
      'b'
    ) +
    ...
    kpiSS(
      'Mujeres Atendidas',
      fmt(g.total_m),
      `${pct(g.total_m, g.total_benef)} del total`,
      'cf',
      'f'
    ) +
    kpiSS(
      'Hombres Atendidos',
      fmt(g.total_h),
      `${pct(g.total_h, g.total_benef)} del total`,
      'cm',
      'm'
    ) +
    ...
    kpiSS(
      'Programas Activos',
      fmt(g.total_prog),
      `${g.total_inst} instituciones participantes`,
      'cg',
      'g'
    ) +
    kpiSS(
      'Localizables',
      fmt(loc.total),
      `${pct(loc.total, g.total_benef)} de beneficiarios únicos`,
      'cgr',
      'gr'
    ) +
    kpiSS(
      'Cobertura Estatal',
      pct(g.total_benef, D._meta.pob_estatal),
      `de ${fmt(D._meta.pob_estatal)} hab.`,
      'cb',
      'b'
    );
  ...
  document.getElementById('pp-cob-pct') &&
    (document.getElementById('pp-cob-pct').textContent = pct(g.total_benef, D._meta.pob_estatal));
  document.getElementById('pp-cob-deck') &&
    (document.getElementById('pp-cob-deck').innerHTML =
      `<strong>${fmt(g.total_benef)}</strong> beneficiarios de ${fmt(D._meta.pob_estatal)} habitantes.`);
  document.getElementById('pp-vul-pct') &&
    (document.getElementById('pp-vul-pct').textContent = pct(
      g.total_benef,
      D._meta.pob_vulnerable
    ));
  document.getElementById('pp-vul-deck') &&
    (document.getElementById('pp-vul-deck').innerHTML =
      `de ${fmt(D._meta.pob_vulnerable)} en situación vulnerable.`);
  ...
  const vulM =
    D.grupos_vulnerables?.mujeres?.pob_vulnerable ||
    Math.round(D._meta.pob_vulnerable * (g.total_m / g.total_benef));
  const vulH =
    D.grupos_vulnerables?.hombres?.pob_vulnerable ||
    Math.round(D._meta.pob_vulnerable * (g.total_h / g.total_benef));
  ...
  if (kpiCob) {
    kpiCob.innerHTML =
      ...
      kpiSS(
        'Pob. Vulnerable',
        pct(g.total_benef, D._meta.pob_vulnerable),
        `${fmt(D._meta.pob_vulnerable)} en situación vulnerable`,
        'cr',
        'r'
      );
  }
  ...
  if (kpiVul) {
    kpiVul.innerHTML =
      kpiSS(
        'Mujeres Vulnerables',
        pct(ateM, vulM),
        `${fmt(vulM)} en situación vulnerable`,
        'cf',
        'f'
      ) +
      kpiSS(
        'Hombres Vulnerables',
        pct(ateH, vulH),
        `${fmt(vulH)} en situación vulnerable`,
        'cm',
        'm'
      );
  }
  ...
  if (kpiS2Ap) {
    kpiS2Ap.innerHTML =
      ...
      kpiSS(
        'Promedio x Benef.',
        (g.total_apoyos / g.total_benef).toFixed(2),
        'apoyos por persona',
        'ck',
        ''
      );
  ...
  if (kpiS2Mn) {
    ...
    kpiS2Mn.innerHTML =
      kpiSS(
        'Menor Cobertura',
        botMunKpi ? botMunKpi.nombre : '—',
        botMunKpi ? fmt(botMunKpi.total) + ' benef.' : '—',
        'ck',
        ''
      ) +
      kpiSS(
        'Mayor Cobertura',
        topMunKpi ? topMunKpi.nombre : '—',
        topMunKpi ? fmt(topMunKpi.total) + ' benef.' : '—',
        'cr',
        'r'
      );
  }
  ...
  if (document.getElementById('pp-inst-num'))
    document.getElementById('pp-inst-num').textContent = g.total_inst;
  ...
  ppL.textContent = '0';
  animCount(ppL, loc.total);
  const locPctVal = ((loc.total / g.total_benef) * 100).toFixed(1);
  document.getElementById('pp-loc-deck').textContent =
    `${locPctVal}% del padrón con datos verificados.`;
  ...
  const apoyosInstRows = Object.entries(D.instituciones)
    .map(([k, v]) => ({ name: k, val: v.apoyos }))
  ...
  ppM.textContent = '0';
  animCount(ppM, g.total_m);
  document.getElementById('pp-mujeres-pct-txt').textContent =
    `${pct(g.total_m, g.total_benef)} del total del padrón`;
  const ppH = document.getElementById('pp-hombres-hero');
  ppH.textContent = '0';
  animCount(ppH, g.total_h);
  document.getElementById('pp-hombres-pct-txt').textContent =
    `${pct(g.total_h, g.total_benef)} del total del padrón`;
  ...
  document.getElementById('donut-sexo-main') &&
    (document.getElementById('donut-sexo-main').innerHTML = buildDonut(
      '',
      g.total_benef,
      slicesSexo
    ));
  ...
  const instRows = Object.entries(D.instituciones)
    .map(([k, v]) => ({ name: k, val: v.total }))
    .sort((a, b) => b.val - a.val)
    .slice(0, 7);
  barList('bar-inst-general', instRows, 'bf-blue', g.total_benef);
  // Sección 3: top 7 localizables por institución
  const locInstRows = (D.localizables?.por_institucion || [])
    .map((x) => ({ name: x.nombre, val: x.total }))
    .sort((a, b) => b.val - a.val)
    .slice(0, 7);
  ...
  const edadRows = [
    ...D.rangos_edad
      .filter((r) => r.key !== 'sin_datos')
      .map((r) => ({ name: r.label, val: r.total })),
  ...
  if (locSexoHero) {
    locSexoHero.textContent = '0';
    animCount(locSexoHero, locM3);
  }
  const locSexoHeroH = document.getElementById('pp-loc-sexo-hero-h');
  if (locSexoHeroH) {
    locSexoHeroH.textContent = '0';
    animCount(locSexoHeroH, locH3);
  }

  // Texto col der
  const locSexoCuerpo = document.getElementById('pp-loc-sexo-cuerpo');
  if (locSexoCuerpo) {
    const pctMloc = pct(locM3, loc.total),
      pctHloc = pct(locH3, loc.total);
    const pctMgen = pct(g.total_m, g.total_benef),
      pctHgen = pct(g.total_h, g.total_benef);
  ...
  const pyrLocData = (D.rangos_edad || [])
    .filter((r) => r.key !== 'sin_datos')
    .map((r) => ({
      key: r.key,
      m: r.m || 0,
  ...
  if (locPirTexto) {
    ...
    const r5064_3 = re3_2.find((r) => r.key === '50-64'),
      r65_3 = re3_2.find((r) => r.key === '65+');
  ...
  if (kpiS3Loc) {
    ...
    kpiS3Loc.innerHTML =
      kpiSS(
        'Localizables',
        fmt(loc.total),
        pct(loc.total, g.total_benef) + ' del padrón',
        'cg',
        'g'
      ) +
      kpiSS(
        'No Localizables',
        fmt(noLoc3),
        pct(noLoc3, g.total_benef) + ' sin contacto',
        'cr',
        'r'
      );
  }
...
// Helpers globales
function instAcc(n) {
  return (INST_COLORS[n] || { acc: '#64748B' }).acc;
}
function instInk(n) {
  return (INST_COLORS[n] || { ink: '#8b949e' }).ink;
}
function instBg(n) {
  return (INST_COLORS[n] || { bg: 'rgba(139,148,158,.15)' }).bg;
}
...
const INST_META = {
  SALUD: {
    ...
    muns: 67,
    titular:
      'MediChihuahua es el único programa con presencia en los 67 municipios del estado y el de mayor cobertura del padrón estatal',
    highlight: 'MediChihuahua',
    img: 'imagenes/inst-salud.png',
    caption: 'MediChihuahua — cobertura universal en los 67 municipios del estado, 2026.',
  },
  SDHyBC: {
    av: '#1D9E75',
    fullname: 'Sec. Desarrollo Humano y Bien Común',
    muns: 63,
    titular:
      'La Secretaría de Desarrollo Humano lidera la diversidad programática con la mayor variedad de programas sociales activos del sistema',
    highlight: 'Secretaría de Desarrollo Humano',
    img: 'imagenes/inst-sdhybc.jpg',
    caption: 'Programas de desarrollo humano y participación ciudadana — SDHyBC, 2026.',
  },
  DIF: {
    av: '#EA580C',
    fullname: 'DIF Chihuahua',
    muns: 33,
    titular:
      'El DIF Chihuahua concentra la mayor proporción de mujeres beneficiarias, reflejo de su orientación hacia el desarrollo y bienestar familiar',
    highlight: 'DIF Chihuahua',
    img: 'imagenes/inst-dif.jpg',
    caption: 'Operativo de atención familiar — DIF Chihuahua, 2026.',
  },
  SPyCI: {
    av: '#059669',
    fullname: 'Sec. Pueblos y Comunidades Indígenas',
    muns: 25,
    titular:
      'La Secretaría de Pueblos y Comunidades Indígenas garantiza la asistencia social a comunidades originarias en {{MUNS}} municipios del estado',
    highlight: 'Secretaría de Pueblos y Comunidades Indígenas',
    img: 'imagenes/inst-spyci.jpg',
    caption: 'Asistencia social para la población indígena chihuahuense — SPyCI, 2026.',
  },
  ICHIJUV: {
    av: '#7C3AED',
    fullname: 'Inst. Chihuahuense de la Juventud',
    muns: 9,
    titular:
      'El Instituto Chihuahuense de la Juventud atiende a jóvenes chihuahuenses con el programa de mayor equilibrio de género del sistema',
    highlight: 'Instituto Chihuahuense de la Juventud',
    img: 'imagenes/inst-ichijuv.avif',
    caption: 'Programa de atención integral a la juventud chihuahuense — ICHIJUV, 2026.',
  },
  ICHDII: {
    av: '#DC2626',
    fullname: 'Inst. Chih. de Desarrollo Integral de la Infancia',
    muns: 1,
    titular:
      'El Instituto Chihuahuense de Desarrollo Integral de la Infancia opera las Estancias Infantiles para el desarrollo pleno de la niñez chihuahuense',
    highlight: 'Instituto Chihuahuense de Desarrollo Integral de la Infancia',
    img: 'imagenes/inst-ichdii.webp',
    caption:
      'Programa de Estancias Infantiles para el Desarrollo Integral de la Niñez — ICHDII, 2026.',
  },
  CULTURA: {
    av: '#CA8A04',
    fullname: 'Secretaría de Cultura',
    muns: 5,
    titular:
      'La Secretaría de Cultura promueve el acceso a expresiones culturales y recreativas para la población chihuahuense',
  ...
  COESPO: {
    av: '#0E7490',
    fullname: 'Comisión Estatal de Población',
    muns: 2,
    titular:
      'La COESPO atiende a migrantes en tránsito y connacionales retornados en los principales puntos de recepción del estado: Chihuahua y Juárez',
...
INST_META.CECYTECH = INST_META.CECYTECH || {
  av: '#64748B',
  fullname: 'Colegio de Estudios Científicos y Tecnológicos del Estado de Chihuahua',
  muns: 0,
  titular: '',
  highlight: '',
  img: '',
...
INST_META.COESVI = INST_META.COESVI || {
  av: '#64748B',
  fullname: 'Comisión Estatal de Vivienda',
  muns: 0,
  titular: '',
  highlight: '',
  img: '',
...
INST_META.ICHIMUJ = INST_META.ICHIMUJ || {
  av: '#64748B',
  fullname: 'Instituto Chihuahuense de las Mujeres',
  muns: 0,
  titular: '',
  highlight: '',
  img: '',
...
INST_META.RURAL = INST_META.RURAL || {
  av: '#64748La Secretaría de Educación del Estado de Chihuahua atiende a la niñez en edad escolar a través del programa de Cobertura de Educación Básica Inclusiva.',
  fullname: 'Secretaría de Desarrollo Rural',
  muns: 0,
  titular: '',
  highlight: '',
  img: '',
...
INST_META.ICHD = INST_META.ICHD || {
  av: '#64748B',
  fullname: 'Instituto Chihuahuense del Deporte',
  muns: 0,
  titular:
    'El Instituto Chihuahuense del Deporte impulsa la activación física de la niñez y juventud chihuahuense a través de Apoyos al Deporte',
  highlight: 'Instituto Chihuahuense del Deporte',
  img: 'imagenes/inst-ichd.jpg',
  caption: 'Apoyos al Deporte — Instituto Chihuahuense del Deporte, 2026.',
};
INST_META.SEECH = INST_META.SEECH || {
  av: '#64748B',
  fullname: 'Secretaría de Educación del Estado de Chihuahua',
  muns: 0,
  titular: '',
  highlight: 'Secretaría de Educación del Estado de Chihuahua',
  img: 'imagenes/inst-seech.jpg',
  caption: 'Cobertura de Educación Básica Inclusiva — SEECH, 2026.',
};
INST_META['SEyD'] = INST_META['SEyD'] || {
  av: '#64748B',
  fullname: 'Secretaría de Educación y Deporte',
  muns: 0,
  titular: '',
  highlight: '',
  img: '',
...
INST_META.TRABAJO = INST_META.TRABAJO || {
  av: '#64748B',
  fullname: 'Secretaría del Trabajo',
  muns: 0,
  titular:
    'La Secretaría del Trabajo impulsa la inserción laboral en el estado con Apoyos para el Empleo, un programa cuya población beneficiaria es mayoritariamente masculina',
  highlight: 'Secretaría del Trabajo',
  img: 'imagenes/inst-trabajo.jpg',
  caption: 'Apoyos para el Empleo en el Estado de Chihuahua — Secretaría del Trabajo, 2026.',
};
INST_META.TURISMO = INST_META.TURISMO || {
  av: '#64748B',
  fullname: 'Secretaría de Turismo',
  muns: 0,
  titular: '',
  highlight: '',
  img: '',
...
function instTitular(k, v, meta) {
  ...
  if (meta.titular) {
    ...
    const tit = meta.titular.replace('{{MUNS}}', meta.muns);
  ...
  if (nProg === 1)
    return (
      ...
      ' opera en ' +
      meta.muns +
      ' municipios del estado a través del programa ' +
      prog0
    );
  const genTxt =
    pctM >= 60
      ...
      : pctM <= 40
        ...
        : 'con distribución equilibrada entre géneros';
  return (
    hl(meta.fullname) +
    ' opera ' +
    nProg +
    ' programas activos en ' +
    meta.muns +
    ' municipios, atendiendo a una población ' +
    genTxt
  );
}
const RANGOS_LABELS = {
